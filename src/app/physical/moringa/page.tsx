                ...
                <table className="w-full text-left border-collapse min-w-[600px]">
                    ...
                    <tbody className={`${FONT.body} text-white/80`}>
                        {[
                            {
                                spec: "Type/Variety",
                                fresh: "PKM1 / PKM2 (Tender)",
                                frozen: "IQF Leaf (Blanched)",
                                dehydrated: "Air-Dried / Freeze-Dried"
                            },
                            {
                                spec: "Grade & Quality",
                                fresh: "A Grade, Dark Green",
                                frozen: "A Grade, Stem-Free",
                                dehydrated: "Premium Food Grade, 100% Pure"
                            },
                            {
                                spec: "Shelf Life",
                                fresh: "2-4 days (Refrigerated)",
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                        ...
                        <table className="w-full text-left border-collapse">
                            <tbody className={`${FONT.body} text-white/80`}>
                                {[
                                    ...
                                    { label: "Protein", value: "25.0 g" },
                                    { label: "Fiber", value: "24.0 g" },
                                    { label: "Vitamin A", value: "180% DV" },
                                    { label: "Calcium", value: "150% DV" },
                                ].map((row, i) => (
                                    <tr key={i} className="border-b border-white/10 last:border-b-0 hover:bg-white/5 transition-colors">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {[
                            { title: "Wellness", icon: HeartPulse, desc: "Nutrient-dense supplements" },
                            { title: "Teas", icon: CupSoda, desc: "Herbal infusions" },
                            { title: "Skincare", icon: Droplets, desc: "Rejuvenating oils & masks" },
                        ].map((card, i) => (
                            <div
                                key={i}
            ...
            <div className="w-full">
                ...
                <div className="border border-white/20 rounded-2xl overflow-hidden overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[600px]">
                        ...
                        <tbody className={`${FONT.body} text-white/80`}>
                            {[
                                { form: "Moringa Powder (Dehydrated)", hsn: "0712 90 90" },
                                { form: "Fresh Moringa Leaves", hsn: "0709 99 90" },
                                { form: "Frozen Moringa Leaves", hsn: "0710 80 90" },
                            ].map((row, i) => (
                                <tr key={i} className="border-b border-white/10 last:border-b-0 hover:bg-white/5 transition-colors">
                    ...
                    >
                        <div className="prose prose-lg prose-invert">
                            ...
                            <p className={`text-white/80 ${FONT.body} leading-relaxed text-lg`}>
                                Known as the "Miracle Tree," our Organic Moringa is a powerhouse of essential nutrients, vitamins, and minerals. Sourced solely from Tamil Nadu, we ensure the highest potency in every leaf.
                            </p>
                            <p className={`text-white/80 ${FONT.body} leading-relaxed text-lg mt-4`}>
                                Available in fresh, frozen, and dried forms, our Moringa is perfect for boosting immunity, improving digestion, and enhancing overall vitality.
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-8">
                            <div>
                                ...
                                <span className={`text-xl font-bold ${FONT.header}`}>Tamil Nadu, India</span>
                            </div>
                            <div>
                                <span className="block text-xs text-[#F59E0B] uppercase tracking-widest mb-1">Grade</span>
                                <span className={`text-xl font-bold ${FONT.header}`}>Superfood Grade</span>
                            </div>
                        </div>
                    </motion.div>
                </div>
