                ...
                <table className="w-full text-left border-collapse min-w-[600px]">
                    <thead className="bg-white/5 text-[#F59E0B]">
                        <tr>
                            ...
                            <th className={`p-4 font-bold uppercase ${FONT.header} border-b border-r border-white/10`}>Fresh (Rhizome)</th>
                            <th className={`p-4 font-bold uppercase ${FONT.header} border-b border-r border-white/10`}>Frozen (Slices/Paste)</th>
                    ...
                    <tbody className={`${FONT.body} text-white/80`}>
                        {[
                            {
                                spec: "Type/Variety",
                                fresh: "Rajapuri / Salem / Waigaon",
                                frozen: "IQF Slices / Puree",
                                dehydrated: "High Curcumin (Lakadong)"
                            },
                            {
                                spec: "Grade & Quality",
                                fresh: "A Grade, Firm Skin, Earthy",
                                frozen: "A Grade, Blanched",
                                dehydrated: "Premium Food Grade (>5% Curcumin)"
                            },
                            {
                                spec: "Shelf Life",
                                fresh: "1-2 weeks (Refrigerated)",
                                frozen: "18-24 months (-18°C)",
                                dehydrated: "2-3 years (Cool Dry Place)"
                            },
                            {
                                spec: "Packaging",
                                fresh: "50kg Gunny Bags",
                                frozen: "LDPE Packs (1kg)",
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                        ...
                        <table className="w-full text-left border-collapse">
                            <tbody className={`${FONT.body} text-white/80`}>
                                {[
                                    ...
                                    { label: "Curcumin Content", value: "> 5.0%" },
                                    { label: "Dietary Fiber", value: "21.0 g" },
                                    { label: "Protein", value: "8.0 g" },
                                    { label: "Iron", value: "41.4 mg" },
                                ].map((row, i) => (
                                    <tr key={i} className="border-b border-white/10 last:border-b-0 hover:bg-white/5 transition-colors">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {[
                            { title: "Culinary", icon: Flame, desc: "Curries & Stews" },
                            { title: "Golden Milk", icon: CupSoda, desc: "Immunity Booster" },
                            { title: "Medicinal", icon: HeartPulse, desc: "Anti-inflammatory" },
                        ].map((card, i) => (
                            <div
                                key={i}
            ...
            <div className="w-full">
                ...
                <div className="border border-white/20 rounded-2xl overflow-hidden overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[600px]">
                        ...
                        <tbody className={`${FONT.body} text-white/80`}>
                            {[
                                { form: "Turmeric Powder", hsn: "0910 30 30" },
                                { form: "Fresh/Raw Turmeric", hsn: "0910 30 10" },
                                { form: "Frozen Turmeric", hsn: "0710 80 90" },
                            ].map((row, i) => (
                                <tr key={i} className="border-b border-white/10 last:border-b-0 hover:bg-white/5 transition-colors">
                    ...
                    >
                        <div className="prose prose-lg prose-invert">
                            ...
                            <p className={`text-white/80 ${FONT.body} leading-relaxed text-lg`}>
                                Our high-curcumin Turmeric Powder is sourced from the fertile regions of Andhra Pradesh and Meghalaya (Lakadong). Renowned for its potent anti-inflammatory properties and distinct earthy aroma, it is the highest quality turmeric available.
                            </p>
                            <p className={`text-white/80 ${FONT.body} leading-relaxed text-lg mt-4`}>
                                Carefully ground to preserve its essential oils and vibrant color, it is an essential ingredient for culinary excellence and holistic health.
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-8">
                            <div>
                                ...
                                <span className={`text-xl font-bold ${FONT.header}`}>Andhra Pradesh, India</span>
                            </div>
                            <div>
                                <span className="block text-xs text-[#F59E0B] uppercase tracking-widest mb-1">Grade</span>
                                <span className={`text-xl font-bold ${FONT.header}`}>High Curcumin (5%+)</span>
                            </div>
                        </div>
                    </motion.div>
                </div>
